...
class CollectTerminalCacheTest(unittest.TestCase):
    ...
    def test_list_run_query_failure_does_not_retry_same_page_forever(self) -> None:
        original = collect.run_merlin
        calls = []
        try:
            def fail(command, payload, **kwargs):
                calls.append((command, payload))
                raise RuntimeError("malformed Merlin response")

            collect.run_merlin = fail
            rows = collect.list_runs("pretraining_test_collect_failure", 100)
        finally:
            collect.run_merlin = original
        self.assertEqual(rows, [])
        self.assertEqual(len(calls), len(collect.stamp_query_terms("pretraining_test_collect_failure")))
