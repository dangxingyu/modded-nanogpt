...
def get_lr(step: int):
    if step < 1000:
        return 2.5
    cool_down_step = int((1 - args.cooldown_frac) * args.num_iterations)
    if step < cool_down_step:
        return (1 - (step - 1000) / (cool_down_step - 1000)) * 1.5 + 1.0
    else:
        return (args.num_iterations - step) / (args.num_iterations - cool_down_step) * 1.0
