...
class Hyperparameters:
    ...
    cooldown_frac_adamw = 0.6 # fraction of training spent cooling down the learning rate for AdamW
    lr_scheduling = "cosine" # learning rate scheduling: "linear" or "cosine"
    # architecture
    vocab_size = 50257
    muon_lr = 0.01
    wd_mul = 2.0
    # ema
    ema_decay = 0.85 # decay rate for EMA
...
parser.add_argument('--ema_decay', type=float, default=args.ema_decay, help="Exponential Moving Average decay")
...
class ModelEMA:
    ...
    def __init__(self, model, decay=0.999):
        self.decay = decay
        # Create a clean copy of the model for EMA
        self.ema_model = copy.deepcopy(model)
        self.ema_model.eval()
        # Ensure EMA parameters don't require gradients
    ...
    def update(self, model):
        ...
        for ema_param, param in zip(self.ema_model.parameters(), model.parameters()):
            ...
            # lerp_ implementation: ema_param.lerp_(param, 1 - decay)
            # This is equivalent to: ema_param = ema_param + (1 - decay) * (param - ema_param)
            ema_param.lerp_(param, 1.0 - self.decay)
            
        # If there are buffers (e.g. for BatchNorm, though not used here), they should be synced
        # GPT/Transformer usually only has causal mask buffers which are constant, but good practice to keep them synced
...
args.ema_decay = cmd_args.ema_decay
...
# It must be initialized after model broadcasting to ensure consistency across ranks
...
ema.ema_model = torch.compile(ema.ema_model, dynamic=False)
...
for step in range(train_steps + 1):
    ...
    if last_step or (args.val_loss_every > 0 and step % args.val_loss_every == 0):
        ...
        # Evaluate EMA model
        # We use the ema.ema_model directly. Note that it's already in eval mode.
        val_loader = distributed_data_generator(args.val_files, val_batch_size, rank, world_size)
        val_loss_ema = 0
        with torch.no_grad():
            for _ in range(val_steps):
                inputs, targets = next(val_loader)
                val_loss_ema += ema.ema_model(inputs, targets, get_window_size_blocks(step))
        val_loss_ema /= val_steps
        del val_loader
        dist.all_reduce(val_loss_ema, op=dist.ReduceOp.AVG)
        print0(f"step:{step}/{train_steps} val_loss_ema:{val_loss_ema:.6f}", console=True)
    ...
    if last_step:
        if master_process and args.save_checkpoint:
            log = dict(step=step, code=code, model=model.state_dict(), ema=ema.ema_model.state_dict(), optimizers=[opt.state_dict() for opt in optimizers])
    ...
    if args.save_checkpoint_step != -1 and step + 1 == args.save_checkpoint_step:
        if master_process:
            log = dict(step=step+1, code=code, model=model.state_dict(), ema=ema.ema_model.state_dict(), optimizers=[opt.state_dict() for opt in optimizers])
            os.makedirs(f"logs/{run_id_full}", exist_ok=True)
