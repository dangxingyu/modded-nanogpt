...
parser.add_argument('--velocity_momentum', type=float, default=args.velocity_momentum)
parser.add_argument('--gemma_lr', type=float, default=args.gemma_lr)
...
@torch.compile
def update(acc_bf16_view_u16: Tensor, mantissa: Tensor, momentum_buffer: Tensor, velocity_buffer: Tensor, grad: Tensor, momentum: Tensor, velocity_momentum: Tensor, eff_lr: Tensor, eff_weight_decay: Tensor):
    assert acc_bf16_view_u16.dtype == mantissa.dtype == torch.uint16
    grad = grad.float()
    momentum_buffer.copy_(momentum * momentum_buffer + (1 - momentum) * grad)
    velocity_buffer.copy_(velocity_momentum * velocity_buffer + (1 - velocity_momentum) * grad * grad)
...
class Muon(torch.optim.Optimizer):
    ...
    """
    def __init__(self, params, lr=0.02, weight_decay=0.01, momentum=0.95, velocity_momentum=0.98, rank=0, world_size=1):
        self.rank = rank
        self.world_size = world_size
        defaults = dict(lr=lr, weight_decay=weight_decay, momentum=momentum, velocity_momentum=velocity_momentum)
    ...
    def step(self):
        ...
        for group in self.param_groups:
            ...
            velocity_momentum = torch._as_tensor_fullprec(group["velocity_momentum"])
            for base_i in range(len(params))[::self.world_size]:
                if base_i + self.rank < len(params):
                    ...
                    update(
                        ...
                        p.grad, momentum, velocity_momentum,
...
class Block(nn.Module):
    def __init__(self, dim: int, num_heads: int, max_seq_len: int, layer_idx: int):
        ...
        self.o_norm = RMSNorm(dim) if self.attn is not None else None

    def forward(self, x: Tensor, ve: Tensor | None, x0: Tensor, block_mask: BlockMask, lambdas: Tensor, sa_lambdas: Tensor):
        x = lambdas[0] * x + lambdas[1] * x0
        if self.attn is not None:
            x = x + self.o_norm(self.attn(x, ve, block_mask, sa_lambdas))
        x = x + self.post_norm(self.mlp(self.mlp_norm(x)))
        return x
...
adam_param_groups = [dict(params=head_params, lr=1/320), dict(params=embed_params, lr=0.3), dict(params=scalar_params, lr=0.015), dict(params=norm_params, lr=args.gemma_lr)]
# small adam epsilon by @YouJiacheng. this is an alternate method of fixing the world_size dependence
# discovered by @fernbear.bsky.social https://x.com/hi_tysam/status/1879692937589875094
optimizer1 = torch.optim.AdamW(adam_param_groups, betas=(0.8, 0.95), eps=1e-10, weight_decay=0.0, fused=True)
optimizer2 = Muon(hidden_matrix_params, lr=args.muon_lr, momentum=0.95, velocity_momentum=args.velocity_momentum, rank=rank, world_size=world_size)
