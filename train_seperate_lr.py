...
def init_linear_small(w: Tensor):
    std = 0.02 * (w.size(-1) ** -0.5) # 0.5 is a bit better than the default 1/sqrt(3)
    bound = (3 ** 0.5) * std
    return w.uniform_(-bound, bound)
...
class GPT(nn.Module):
    def __init__(self, vocab_size: int, num_layers: int, num_heads: int, model_dim: int, max_seq_len: int):
        ...
        # self.lm_head_w = nn.Parameter(init_linear(torch.empty(next_multiple_of_n(vocab_size, n=128), model_dim))) 
        self.lm_head_w = nn.Parameter(init_linear_small(torch.empty(next_multiple_of_n(vocab_size, n=128), model_dim))) # using small init for lm_head_w
...
parser.add_argument('--zero_step_lr', type=float, default=args.zero_step_lr)
...
args.zero_step_lr = cmd_args.zero_step_lr
...
if master_process:
    metadata = {
        ...
        "hyperparameters": {
            ...
            "zero_step_lr": args.zero_step_lr,
...
def get_lr(step: int):
    if step < 2580:
        lr = 3.541946e+00/(step+1) + 4.445067e-03
    else:
        lr = -1.390586e-06 * step + 9.010130e-03
    return min(lr, 0.05) / 0.05
