        ...
        for group in self.param_groups:
            for p in group["params"]:
                ...
                if not torch.isfinite(grad).all():
                    print(f"Warning: Non-finite gradient detected for param shape {p.shape}. Skipping step.")
                    return loss
                ...
                exp_avg.mul_(beta1).add_(grad.float(), alpha=(1.0 - beta1))
                exp_avg_sq.mul_(beta2).add_(grad_projected.float().square(), alpha=(1.0 - beta2))

                # Check for NaNs in exp_avg_sq
                if not torch.isfinite(exp_avg_sq).all():
                     print(f"Warning: Non-finite exp_avg_sq detected for param shape {p.shape}. Resetting to zeros.")
                     exp_avg_sq.zero_()
                ...
                norm_dims = (-2, -1) if p.ndim >= 2 else None
                p_norm = p.norm(dim=norm_dims, keepdim=True, p='fro')
                grad_norm = norm_grad.norm(dim=norm_dims, keepdim=True, p='fro') + 1e-30

                p.add_(norm_grad * (-step_size * (p_norm / grad_norm)))
                new_p_norm = p.norm(dim=norm_dims, keepdim=True, p='fro')
                p.mul_(p_norm / new_p_norm)
    ...
    def update_preconditioner(self, grad, state, 
        ...
        if grad.dim() == 1:
            if precondition_1d and grad.shape[0] <= max_precond_dim:
                state['GG'][0].lerp_((grad.unsqueeze(1) @ grad.unsqueeze(0)).to(dtype=state['GG'][0].dtype), 1-state['shampoo_beta'])
        else:
            if merge_dims:
                ...
                for idx, sh in enumerate(new_grad.shape):
                    if sh <= max_precond_dim:
                        ...
                        state['GG'][idx].lerp_(outer_product.to(dtype=state['GG'][idx].dtype), 1-state['shampoo_beta'])
            else:
