...
@torch.compile
def update_2(acc_bf16_view_u16: Tensor, mantissa: Tensor, momentum_buffer: Tensor, grad: Tensor, momentum: Tensor, eff_lr: Tensor, eff_weight_decay: Tensor):
    assert acc_bf16_view_u16.dtype == mantissa.dtype == torch.uint16
    grad = grad.float()
    momentum_buffer.copy_(momentum * momentum_buffer + (1 - momentum) * grad)
    v = zeropower_via_newtonschulz5(momentum * momentum_buffer + (1 - momentum) * grad)
...
class CausalSelfAttention(nn.Module):
    def __init__(self, dim: int, num_heads: int, max_seq_len: int, head_dim=128):
        ...
        self.qkvo_w.detach()[3].zero_() # out zero init suggested by @Grad62304977 - DISABLED: using normal init - Enabled again
...
class MLP(nn.Module):
    def __init__(self, dim: int):
        ...
        self.proj_w = nn.Parameter(torch.zeros(dim, hdim).bfloat16()) # Changed back to zeros
...
class GPT(nn.Module):
    def __init__(self, vocab_size: int, num_layers: int, num_heads: int, model_dim: int, max_seq_len: int):
        ...
        self.lm_head_w = nn.Parameter(torch.zeros(next_multiple_of_n(vocab_size, n=128), model_dim)) # Changed back to zeros
...
parser.add_argument('--phase_2_ratio', type=float, default=args.phase_2_ratio)
cmd_args = parser.parse_args()
args.muon_lr = cmd_args.muon_lr
args.cooldown_frac = cmd_args.cooldown_frac
args.phase_2_ratio = cmd_args.phase_2_ratio
...
if master_process:
    metadata = {
        ...
        "hyperparameters": {
            ...
            "phase_2_ratio": args.phase_2_ratio,
        },
        "model": {
            "num_layers": 16,
            "num_heads": 8,
...
def get_lr(step: int):
    # New logic:
    # Before cooldown: muon_lr
    # During cooldown: linear decay from phase_2_ratio * muon_lr to 0
    x = step / args.num_iterations # progress in training
    assert 0 <= x <= 1
    if x < args.cooldown_frac:
        return 1.0
    else:
        return (1 - x) / args.cooldown_frac * args.phase_2_ratio


def get_lr_adam(step: int):
    x = step / args.num_iterations # progress in training
    assert 0 <= x < 1
    ...
    for opt in optimizers:
        for group in opt.param_groups:
            if isinstance(opt, torch.optim.AdamW):
                group["lr"] = group["initial_lr"] * get_lr_adam(step)
            else:
